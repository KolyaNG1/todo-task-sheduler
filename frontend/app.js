...
const state = {
  ...
  section: 'plan', report: null, dailyProgress: null, calendarDays: 21, templateSlots: [], goals: [], goalDetail: null, goalTaskIds: [], deletedTasks: [], sessionView: localStorage.getItem('planner-session-view') || 'timeline', taskSessionGroups: null, panelHidden: localStorage.getItem('planner-task-panel-hidden') === 'true', navPanelHidden: localStorage.getItem('planner-nav-panel-hidden') === 'true', taskSort: localStorage.getItem('planner-task-sort') || 'deadline', availabilitySelection: null, dragSource: null,
...
function render() { renderHeader(); renderDailySuccess(); renderGoalStrip(); renderTaskPanelVisibility(); renderTasks(); renderWeek(); renderSession(); renderPlan(); renderSection(); }
...
function renderGoalStrip() { const root = $('#goal-strip'); const goals = state.goals.filter((goal) => goal.status === 'ACTIVE').slice(0, 4); root.hidden = state.section !== 'plan' || goals.length === 0; if (root.hidden) return; root.innerHTML = `<div class="goal-strip-title"><span>🎯</span><strong>Цели в фокусе</strong><button class="button ghost" data-action="open-goals">Все цели →</button></div><div class="goal-strip-list">${goals.map((goal) => `<button class="goal-mini-card" data-action="open-goal" data-id="${goal.id}" style="--goal-color:${goal.color || '#356AE6'}"><span>${escapeHtml(goal.title)}</span><strong>${goal.progress_percent || 0}%</strong><i><b style="width:${goal.progress_percent || 0}%"></b></i><small>${goal.completed_task_count}/${goal.leaf_task_count || 0} чекпоинтов</small></button>`).join('')}</div>`; }
...
function renderTasks() {
  const tasks = sortTasks(state.tasks.filter((task) => task.status === 'ACTIVE' && task.title.toLowerCase().includes(state.search.toLowerCase()) && (state.filter === 'all' || state.filter === 'unplanned' && task.is_leaf && task.planning_status === 'UNPLANNED' || state.filter === 'overdue' && task.is_overdue)));
  $('#task-count').textContent = state.tasks.filter((task) => task.status === 'ACTIVE').length;
  $('#task-list').innerHTML = tasks.length ? tasks.map((task) => {
    const canPlan = task.is_leaf && task.planning_status !== 'PLANNED'; const depth = taskDepth(task);
    return `<article class="task-card ${state.selected.has(task.id) ? 'selected' : ''} ${task.is_overdue ? 'overdue' : ''} ${task.is_leaf ? '' : 'task-parent'}" style="--task-depth:${depth}" ${canPlan ? `draggable="true" data-drag-task="${task.id}"` : ''} data-task-card="${task.id}">
      ...
      <div><div class="task-name">${task.is_leaf ? '' : '▾ '}${escapeHtml(task.title)}</div><div class="task-meta ${task.is_overdue ? 'danger' : ''}">${task.parent_task_title ? `внутри ${escapeHtml(task.parent_task_title)} · ` : ''}${task.is_leaf ? `${escapeHtml(task.direction?.name || 'Без направления')} · ${task.deadline_at ? deadlineText(task.deadline_at) : 'без срока'}` : `${task.child_count} чекпоинт${task.child_count === 1 ? '' : task.child_count < 5 ? 'а' : 'ов'}`}${task.repeat_rule === 'WEEKLY' ? ' · еженедельно' : ''}</div><div class="task-actions"><button class="task-icon" data-start-task="${task.id}" title="Начать" aria-label="Начать ${escapeHtml(task.title)}">▶</button>${task.is_leaf ? `<button class="task-icon" data-action="auto-plan-task" data-id="${task.id}" ${canPlan ? '' : 'disabled'} title="Распределить автоматически" aria-label="Распределить автоматически">◷</button>` : ''}<button class="task-icon" data-action="new-child-task" data-id="${task.id}" title="Добавить чекпоинт" aria-label="Добавить чекпоинт к ${escapeHtml(task.title)}">＋</button><button class="task-icon" data-action="edit-task" data-id="${task.id}" title="Редактировать" aria-label="Редактировать">✎</button><button class="task-icon task-delete" data-action="delete-task" data-id="${task.id}" title="Удалить задачу" aria-label="Удалить ${escapeHtml(task.title)}">×</button></div></div>
      <span class="task-duration">${task.is_leaf ? `${task.planned_minutes}/${task.estimate_minutes}м` : 'группа'}</span>
    </article>`;
...
async function createGoalTask(form) {
  ...
  try {
    await api(`/workspaces/${state.workspace.id}/tasks`, { method:'POST', body:JSON.stringify({ title:fields.get('title'), goal_id:state.goalDetail.id, parent_task_id:fields.get('parent_task_id') || null, estimate_minutes:estimate, min_block_minutes:Math.min(30, estimate), preferred_block_minutes:estimate, priority:Number(fields.get('priority')), color:state.goalDetail.color || null }) });
    await loadData();
...
function renderSection() {
  ...
  if (state.section === 'tasks') {
    view.innerHTML = `<header class="page-header"><div><p class="eyebrow">ПОЛНЫЙ СПИСОК</p><h2>Все задачи</h2></div><button class="button primary" data-action="new-task">+ Задача</button></header><div class="table-list">${sortTasks(state.tasks).map((task) => `<article class="full-task ${task.is_overdue ? 'overdue' : ''} ${task.is_leaf ? '' : 'task-parent'}" style="--task-depth:${taskDepth(task)}"><span class="task-color" style="background:${colorForTask(task)}"></span><div><strong>${task.is_leaf ? '' : '▾ '}${escapeHtml(task.title)}</strong><p>${task.parent_task_title ? `внутри ${escapeHtml(task.parent_task_title)} · ` : ''}${task.is_leaf ? `${task.direction?.name || 'Без направления'} · ${task.deadline_at ? deadlineText(task.deadline_at) : 'без срока'} · важность ${task.priority}` : `${task.child_count} чекпоинт${task.child_count === 1 ? '' : task.child_count < 5 ? 'а' : 'ов'}`}${task.repeat_rule === 'WEEKLY' ? ' · повторяется еженедельно' : ''}</p></div><span class="status">${task.status === 'ACTIVE' ? task.is_leaf ? task.planning_status : 'ГРУППА' : task.status}</span><div class="row-actions">${task.status === 'ACTIVE' ? `<button class="button ghost" data-action="new-child-task" data-id="${task.id}">+ Чекпоинт</button><button class="button ghost" data-action="edit-task" data-id="${task.id}">Изменить</button><button class="button ghost" data-action="start" data-id="${task.id}">Начать</button>${task.is_leaf ? `<button class="button secondary" data-action="complete" data-id="${task.id}">Готово</button>` : ''}` : `<button class="button secondary" data-action="restore" data-id="${task.id}">Вернуть</button>`}<button class="button ghost" data-action="delete-task" data-id="${task.id}">Удалить</button></div></article>`).join('') || '<div class="empty"><strong>История пуста</strong></div>'}</div>`;
...
function openModal(type, entityId = null, options = {}) {
  ...
  if (type === 'task') {
    ...
    parentInput?.addEventListener('change', () => {
      const parent = state.tasks.find((item) => item.id === parentInput.value);
      if (!parent || entityId) return;
      if (parent.goal_id) goalInput.value = parent.goal_id;
      if (parent.direction?.id) directionInput.value = parent.direction.id;
      $('#modal-content [name="priority"]').value = parent.priority;
      $('#modal-content [name="color"]').value = colorForTask(parent);
      applyParent();
    });
  }
  modal.showModal();
...
async function planSelected() { const selected = state.tasks.filter((task) => state.selected.has(task.id) && task.is_leaf && task.planning_status !== 'PLANNED').map((task) => task.id); if (!selected.length) { showToast('Выберите хотя бы один конечный чекпоинт, который ещё не размещён.', true); return; } if (!state.week.days.reduce((sum, day) => sum + day.free_minutes, 0)) { showToast('Сначала задайте рабочее время.', true); openModal('worktime'); return; } try { const end = new Date(state.weekStart); end.setDate(end.getDate() + 7); state.plan = await api(`/workspaces/${state.workspace.id}/planner/runs`, { method:'POST', body:JSON.stringify({ task_ids:selected, horizon_start:state.weekStart.toISOString(), horizon_end:end.toISOString() }) }); state.planPanelHidden = false; renderWeek(); renderPlan(); } catch (error) { showToast(error.message, true); } }
...
document.addEventListener('click', async (event) => {
  ...
  const taskCard = event.target.closest('[data-task-card]'); if (taskCard && !event.target.closest('button,input')) { const id = taskCard.dataset.taskCard; const task = state.tasks.find((item) => item.id === id); if (task?.is_leaf && task.planning_status !== 'PLANNED') { state.selected.has(id) ? state.selected.delete(id) : state.selected.add(id); renderTasks(); } return; }
