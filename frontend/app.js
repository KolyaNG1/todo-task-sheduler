...
async function loadReport() { try { const start = new Date(`${state.reportWeekStart}T12:00:00`); const end = new Date(start); end.setDate(end.getDate() + 6); const report = await api(`/workspaces/${state.workspace.id}/reports/daily/${state.reportDate}`); const history = await api(`/workspaces/${state.workspace.id}/reports/history?start_date=${state.reportWeekStart}&end_date=${dateKey(end)}`); state.report = report; state.reportHistory = history.days || []; if (state.reportDate === dateKey(new Date())) state.dailyProgress = report; renderDailySuccess(); renderSection(); } catch (error) { showToast(error.message, true); } }
