...
const state = {
  ...
  weekStart: monday(new Date()), week: null, tasks: [], directions: [], projects: [], labels: [], notifications: [], selected: new Set(),
...
async function loadData() {
  ...
  try {
    ...
    state.week = startup.week; state.tasks = startup.tasks; state.projects = startup.projects || startup.directions || []; state.directions = state.projects; state.labels = labels; state.notifications = notifications; state.goals = goals; state.session = startup.active_session; state.templateSlots = startup.default_availability || []; state.dailyProgress = progress;
    render();
  } catch (error) {
...
async function refreshPlanData() {
  ...
  reconcilePendingBlockMutations(startup.week);
  state.week = startup.week; state.tasks = startup.tasks; state.projects = startup.projects || startup.directions || []; state.directions = state.projects; state.session = startup.active_session; state.templateSlots = startup.default_availability || []; render();
...
function openModal(type, entityId = null, options = {}) {
  const modal = $('#modal'); const task = entityId ? state.tasks.find((item) => item.id === entityId) || projectTaskForDetail(entityId) : null; const direction = entityId ? state.projects.find((item) => item.id === entityId) : null; const label = entityId ? state.labels.find((item) => item.id === entityId) : null; const goalItem = entityId ? state.goals.find((item) => item.id === entityId) : null; const eventItem = entityId ? uniqueEvents().find((item) => item.id === entityId) : null; const blockItem = entityId ? state.week?.days.flatMap((item) => item.blocks).find((item) => item.id === entityId) : null; const day = state.week?.days.find((item) => item.date === dateKey(state.weekStart)); const preselectedGoalId = task?.goal_id || options.goalId || ''; const preselectedParentId = task?.parent_task_id || options.parentTaskId || ''; const preselectedProjectId = task?.project_id || task?.direction?.id || options.projectId || '';
...
document.addEventListener('click', async (event) => {
  ...
  const projectCard = event.target.closest('.project-card'); if (projectCard && !event.target.closest('button')) { const projectId = projectCard.querySelector('[data-action="open-project"]')?.dataset.id; if (projectId) openProjectPage(projectId); return; }
