...
def week_response(view: dict) -> dict:
    return {"week_start": view["week_start"].isoformat(), "planning_revision": view["planning_revision"], "grid_step_minutes": view["workspace"].grid_step_minutes, "timezone": view["workspace"].timezone, "deadline_workload": view["deadline_workload"], "days": [{"date": day["date"].isoformat(), "capacity_minutes": day["capacity_minutes"], "free_minutes": day["free_minutes"], "deadline_count": day["deadline_count"], "deadline_tasks": [{**item, "deadline_at": item["deadline_at"].isoformat()} for item in day["deadline_tasks"]], "availability": [interval_view(item) for item in day["availability"]], "fixed_events": [{**event, "local_date": event["local_date"].isoformat() if event["local_date"] else None, "start_at": event["start_at"].isoformat(), "end_at": event["end_at"].isoformat()} for event in day["fixed_events"]], "blocks": [block_view(item) for item in day["blocks"]]} for day in view["days"]]}
